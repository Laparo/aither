...
- **FR-004a** *(Descriptor patterns)*: System MUST fetch text content and media assets for each lesson and generate one material slide per item (`{NN}_{slugifiedDescriptor}.html`). The `slugifiedLessonTitle` is computed as `slugify(lessonTitle) || 'lesson-{lessonIndex}'` where `{lessonIndex}` is the 1-based position in the sorted lesson list — this ensures descriptors never begin with a leading dash when the lesson title slugifies to an empty string. Descriptor patterns by type:
  ...
  - *Example*: A lesson titled `""` (empty) at position 3 → `slugifiedLessonTitle = "lesson-3"` → text descriptor `"lesson-3-text-1"`, image fallback `"lesson-3-image-1"`.
