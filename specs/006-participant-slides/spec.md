...
- **Filename Convention**: All slides use a unified naming scheme: `{NN}_{identifier}[_{firstName}].html`. A global sequence counter (`NN`) determines presentation order. The `identifier` is a slugified descriptor (title, material identifier, or media alt-text). For collection slides (Mode A/B), the participant's first name is appended. Examples: `001_intro.html`, `002_einfuehrung.html`, `005_video-analysis_anna.html`. All sequence numbers are 1-based and zero-padded (3-digit minimum per FR-004b). Alphabetical sort by filename equals presentation order.
  ...
  - **firstName extraction**: Source is `participant.name`. Extract the first word (split on whitespace). Transliterate German umlauts (`ä→ae`, `ö→oe`, `ü→ue`, `ß→ss`). Lowercase, then apply Unicode NFD normalization and strip combining marks to handle remaining diacritics. Remove all characters that are not ASCII letters (`a-z`), digits (`0-9`), or hyphens — this strips apostrophes, periods, commas, parentheses, and any other punctuation. Hyphens in names are preserved (e.g., `Jean-Pierre → jean-pierre`). Collapse consecutive hyphens and trim leading/trailing hyphens. **Fallback**: Use `participant-{index}` (1-based) when `participant.name` is null, empty, whitespace-only, **or** when the processed firstName is an empty string after all normalization steps (e.g., a name consisting entirely of non-ASCII, non-transliterable characters). Examples: `"Anna Müller" → "anna"`, `"Jean-Pierre Dupont" → "jean-pierre"`, `"O'Brien" → "obrien"`, `"Владимир" → "" (after processing) → "participant-1"`, `null → "participant-1"`.
  - **Collision handling**: Collision detection occurs during filename/slug generation, before the global sequence `{NN}` is assigned. Detection: compare slugified values of `CourseMaterial.identifier` (or title/alt-text) across the current generation batch. Resolution order: (1) use the base slug as-is; (2) if a duplicate slug already exists among previously assigned identifiers, append a per-slug incremented suffix `-{n}` (e.g., `agenda-2`, `agenda-3`); (3) the global sequence number `{NN}` provides the final uniqueness guarantee. *Example*: Material A with title "Agenda" → slug `agenda` → `005_agenda.html`; Material B with identifier "agenda" → duplicate detected → slug `agenda-2` → `006_agenda-2.html`.
  - **Null firstName fallback**: When `participant.name` is null, empty, or the processed result is an empty string, use `participant-{index}` where `{index}` is the 1-based position in the participants array.
