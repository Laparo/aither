...
- [x] CHK013 — Is the Mode A file naming `03_material_{topicIdx}_{sectionIdx}_{iterationIdx}.html` clarified with index base (0-based vs 1-based for each segment)? ✅ Added to Technical Constraints: "All filename indices are 1-based and zero-padded to 2 digits (fixed-width, e.g., `01`, `02`, … `99`)". Research R8 example confirms. Note: the global `{NN}` prefix uses 3-digit minimum padding (FR-004b); the per-segment indices (`topicIdx`, `sectionIdx`, `iterationIdx`) use fixed 2-digit padding.
