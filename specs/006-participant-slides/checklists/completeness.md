...
- [x] CHK005 — Is the ordering of slides within `output/slides/{courseId}/` explicitly defined (lexicographic by filename, or by generation order)? ✅ Unified `{NN}_{identifier}[_{firstName}].html` convention — global sequence number determines order; alphabetical sort = presentation order.
...
- [x] CHK013 — Is the unified `{NN}_{identifier}[_{firstName}].html` naming convention documented with global sequence counter? ✅ Added to Technical Constraints. Sequence is 1-based, zero-padded (3-digit min). Participant first name appended for collection slides.
- [x] CHK014 — Is the zero-padding strategy for Mode B filenames defined (fixed 2-digit, or dynamic padding based on record count)? ✅ Contract identifier-distribution test: "3-digit padding because 12 records" — dynamic padding based on record count digits.
...
- [x] CHK027 — Is the US3 acceptance criterion "filename includes the participant's first name" verifiable with concrete examples for multi-section, multi-participant combinations? ✅ US3 example: `{NN}_preparation-sheet_anna.html`. Quickstart §5 shows full directory listing with concrete files.
